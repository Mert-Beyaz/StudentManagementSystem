...
const studentsOfLessonSidebarBtn = document.getElementById("studentsOfLessonSidebarBtn")
const showAllStudentsSidebarBtn = document.getElementById("showAllStudentsSidebarBtn")
const searchStudentSidebarBtn = document.getElementById("searchStudentSidebarBtn")

//Accessing the div they are in to determine when the tables will appear.
let addStudentSidebarBtnDiv = document.getElementById("addStudentSidebarBtnDiv")
let addLessonSidebarBtnDiv = document.getElementById("addLessonSidebarBtnDiv")
let studentOfLessonSidebarBtnDiv = document.getElementById("studentOfLessonSidebarBtnDiv")
let showAllStudentSidebarBtnDiv = document.getElementById("showAllStudentSidebarBtnDiv")
let searchStudentsidebarBtnDiv = document.getElementById("searchStudentsidebarBtnDiv")
...
const saveStudentOfLessonBtn = document.getElementById("saveStudentOfLessonBtn")
...
var sidebarBtnDivList = [addStudentSidebarBtnDiv, addLessonSidebarBtnDiv,
    studentOfLessonSidebarBtnDiv, showAllStudentSidebarBtnDiv, searchStudentsidebarBtnDiv];//A list created to decide which table to display.
...
studentsOfLessonSidebarBtn.addEventListener("click", studentsOfLessonSidebarBtnClick);
showAllStudentsSidebarBtn.addEventListener("click", showAllStudentsSidebarBtnClick);//top butona basınca
...
function selectButton(button) {
    if (saveCheck) {//It's checking to see if there's processing going on here.
...
saveSearchStudentBtn.addEventListener("click", saveSearchStudentBtnClick);
...
function addLessonSubBtnClick(e) {
    ...
    let error = addLessonCheckErrors(lessonID.value, lessonName.value, pointScale.value);//It's checking to see if there are any errors.
...
function creatLessonTable() {
    ...
    for (let i = 0; i < lessonList.length; i++) {
        ...
        tEditButton.innerHTML = "<button class='btnInTable' id='editLessonBtn' onClick = 'editLessonBtnClick(this)';>Edit Lesson</button>";
        tDeleteButton.innerHTML = "<button class='btnInTable' id='deleteLessonBtn' onClick = 'deleteLessonBtnClick(this)';>Delete Lesson</button>";
    }
...
function addLessonCheckErrors(lessonID, lessonName, pointScale) {
    let error = false;
    for (let i = 0; i < lessonList.length; i++) {
        if (lessonID == lessonList[i].lessonID) {//The same ID also checks if you have another lesson.
            alert("The lesson you want to add already exists.")
            error = true;
        }
    }
    if (lessonID == "" || lessonName == "" || pointScale == "") {//It is checking if there is an null value.
...
////////////////////////////////////////////////////////////////
class creatStudent {
    constructor(studentID, fname, surname, lesson, midterm, final, grade, letterGrade) {
...
function addStudentSubBtnClick(e) {
    ...
    let letterGrade = calculateLetterGrade(lesson.value, midterm.value, final.value);//It is calculating the student's letter grade.
    let obj = new creatStudent(studentID.value, fname.value, surname.value, lesson.value,
        midterm.value, final.value, grade, letterGrade)//It creates a new student object.
    let error = addStudentCheckErrors(studentID.value, fname.value, surname.value, lesson.value,
        midterm.value, final.value);//Is's checking to see if there are any errors.
    if (!error) {
...
function calculateGrade(midterm, final) {////It is calculating the student's grade.
    let grade = midterm * 0.40 + final * 0.60
    return grade;
}
...
function calculateLetterGrade(lesson, midterm, final) {
    ...
    if (temporalPointScale == "7") {//The 7-point system calculates the student's letter grade.
    ...
    else if (temporalPointScale == "10") {//The 10-point system calculates the student's letter grade.
...
function calculateGPA(studentID) {
    ...
    return GPA;
}//It calculates the student's GPA.

function creatStudentTable() {
    clearTable(addStudentTable); //It's cleaning table.
    for (let i = 0; i < studentList.length; i++) {
        ...
        tEditButton.innerHTML = "<button class='btnInTable' id='editStudentBtn' onClick = 'editAddStudentBtnClick(this)';>Edit Student Lesson</button>";
        tDeleteButton.innerHTML = "<button class='btnInTable' id='deleteStudentBtn' onClick = 'deleteStudentBtnClick(this)';>Delete Student</button>";
        //It adds Edit and Delete Buttons to the last columns.
    }
}//It is creating the Student table.

function addStudentCheckErrors(studentID, fname, surname, lesson, midterm, final) {
    let error = false;
    for (let i = 0; i < studentList.length; i++) {
        ...
        else if (studentID == studentList[i].studentID && (fname != studentList[i].fname
    ...
    if (studentID == "" || fname == "" || surname == "" ||
        lesson == "" || midterm == "" || final == "") {//It is checking if there is an null value.
        alert("You entered incorrect information.")
        error = true;
    }

    else if (midterm > 100 || midterm < 0 ||
        final > 100 || final < 0) {
...
function creatStudentOfLessonTable() {
    ...
    if (!error) {
        if (rating.value == "Passed") {
            for (let i = 0; i < studentList.length; i++) {
                if (studentList[i].lesson == searchLesson.value && studentList[i].letterGrade != "F") {
                    var row = generalStudentTable(studentsOfLessonTable, i);//It creat general student table.
                    var tEditButton = row.insertCell(7);
                    var tDeleteButton = row.insertCell(8);
                    tEditButton.innerHTML = "<button class='btnInTable' id='editStudentBtn' onClick = 'editStudentOfLessonBtnClick(this)';>Edit Student Score</button>";
                    tDeleteButton.innerHTML = "<button class='btnInTable' id='deleteStudentBtn' onClick = 'deleteStudentBtnClick(this)';>Delete Student</button>";
                    totalGrade += studentList[i].grade;
                }
            }
...
function addClassAverageRow(totalGrade) {
    ...
    row.insertCell(0).innerHTML = "Number of student: "
        + (studentsOfLessonTable.rows.length - 2) + " Grade of selected students: " + classAverage;
...
function creatAllStudentTable() {
    ...
    if (!error) {
        ...
        for (let i = 0; i < studentList.length; i++) {
            ...
            if (checkCounter == 1) {//It controls whether it adds the student more than 1 times.
                var GPA = calculateGPA(studentList[i].studentID) //It calculates the student's GPA.
                ...
                tGPA.innerHTML = GPA;
                tEditButton.innerHTML = "<button class='btnInTable' id='editStudentBtn' onClick = 'editAllStudentBtnClick(this)';>Edit studentID</button>";
                tDeleteButton.innerHTML = "<button class='btnInTable' id='deleteAllStudentBtn' onClick = 'deleteAllDetailsStudentBtnClick(this)';>Delete All Details of Student</button>";
            }
        }
    }
    ...
    if (!error) {
        for (let i = 0; i < studentList.length; i++) {
            if (searchStudentByID.value == studentList[i].studentID) {
                ...
                var tEditButton = row.insertCell(7);
                var tDeleteButton = row.insertCell(8);
                tEditButton.innerHTML = "<button class='btnInTable' id='editStudentBtn' onClick = 'editSearchStudentBtnClick(this)';>Edit Name and Surname</button>";
                tDeleteButton.innerHTML = "<button class='btnInTable' id='deleteStudentBtn' onClick = 'deleteStudentBtnClick(this)';>Delete Student</button>";
            }
        }
...
function searchStudentCheckErrors(studentID) {
    ...
    if (studentID == "") {
        ...
        for (let i = 0; i < studentList.length; i++) {
            error = true;
            if (studentList[i].studentID == studentID) {//Eğer eşitse öğrenci var demek 
                error = false;
                break;
            }
        }
...
function calculateNewLetterAverage(pointScale, midterm, final) {
    let letterGrade;
    let localGrade = calculateGrade(midterm, final);//It is calculating the student's grade.
    if (pointScale == "7") {//The 7-point system calculates the student's letter grade.
        if (localGrade < 101 && localGrade > 92) {
            letterGrade = "A";
        }
        else if (localGrade < 93 && localGrade > 84) {
    ...
    else if (pointScale == "10") {//The 10-point system calculates the student's letter grade.
        if (localGrade < 101 && localGrade > 89) {
            letterGrade = "A";
        }
        else if (localGrade < 90 && localGrade > 79) {
...
function removeOption(lessonName, optionTable) {
    for (var i = 0; i < optionTable.length; i++) {
        if (optionTable.options[i].value == lessonName)
...
function editLessonBtnClick(BTN) {
    if (!saveCheck) {
        ...
        lessonNameInTable.innerHTML = "<input type='text' id='editLessonName'></input>";
        pointScaleInTable.innerHTML = "<select name='pointScale' id='editPointScale'></select>";
        //add option
...
function saveLessonBtnClick(e) {
    ...
    if (saveCheck) {
        ...
        let error = errorCheckSaveAddLessonBtn(newLessonID.value, newLessoName.value, newPointScale.value);//It's checking to see if there are any errors.
        if (!error) {
            for (let i = 0; i < lessonList.length; i++) {
                if (lessonList[i].lessonID == temporalInfoList[0]) {
                    ...
                    for (let j = 0; j < studentList.length; j++) {
                        if (lessonList[i].lessonName == studentList[j].lesson) {
                            ...
                            let newletterGrade = calculateNewLetterAverage(newPointScale.value,
                                studentList[j].midterm, studentList[j].final);
...
function errorCheckSaveAddLessonBtn(lessonID, lessonName, pointScale) {
    let error = false;
    for (let i = 0; i < lessonList.length; i++) {
        if (lessonID == lessonList[i].lessonID && lessonName == lessonList[i].lessonName && pointScale == lessonList[i].pointScale) {
            alert("The lesson you want to add already exists.")
            error = true;
        }
    }
...
function editAddStudentBtnClick(BTN) {
    if (!saveCheck) {
        ...
        lessonInTable.innerHTML = "<select name='pointScale' id='editLesson'></select>"
...
function saveAddStudentBtnClick(e) {
    ...
    if (saveCheck) {
        ...
        let error = errorCheckSaveAddStudentBtn(temporalInfoList[0], newLesson.value);//It's checking to see if there are any errors.
        if (!error) {
            for (let i = 0; i < studentList.length; i++) {
                if (studentList[i].studentID == temporalInfoList[0] && studentList[i].lesson == temporalInfoList[1]) {
                    //It replaces new information with minus information.
                    studentList[i].lesson = newLesson.value;
                    studentList[i].grade = calculateGrade(studentList[i].midterm, studentList[i].final);//It is calculating the student's new grade.
                    studentList[i].letterGrade = calculateLetterGrade(studentList[i].lesson, studentList[i].midterm, studentList[i].final);//It is calculating the student's new letter grade.
                }
            }
            creatStudentTable();//It is creating the Student table.
            saveCheck = false;
        ...
        if (!error) {
            for (let i = 0; i < studentList.length; i++) {
                if (studentList[i].studentID == temporalInfoList[0] && studentList[i].lesson == temporalInfoList[1]) {
                    ...
                    studentList[i].grade = calculateGrade(newMidterm.value, newFinal.value);//It is calculating the student's new grade.
                    studentList[i].letterGrade = calculateLetterGrade(studentList[i].lesson, newMidterm.value, newFinal.value);//It is calculating the student's new letter grade.
                }
            }
            creatStudentOfLessonTable();//It is creating the Student of Lesson table.
            saveCheck = false;
...
function editSearchStudentBtnClick(BTN) {
    if (!saveCheck) {
        ...
        studentFnameInTable.innerHTML = "<input type='text' id='editStudentFname'></input>"
        studentSurnameInTable.innerHTML = "<input type='text' id='editStudentSurname'></input>"
        saveCheck = true;
    }
...
function saveSearchStudentBtnClick(e) {
    ...
    if (saveCheck) {
        ...
        let error = errorCheckSaveSearchStudentBtn(newStudentFname, newStudentLname);//It's checking to see if there are any errors.
        if (!error) {
            for (let i = 0; i < studentList.length; i++) {
...
function editAllStudentBtnClick(BTN) {
    if (!saveCheck) {
        ...
        studentIDInTable.innerHTML = "<input type='text' id='editStudentID'></input>"
        saveCheck = true;
    }
...
function deleteStudentBtnClick(BTN) {
    ...
    for (let i = 0; i < studentList.length; i++) {
        if (studentList[i].studentID == studentIDInTable &&
